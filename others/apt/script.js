...
const MENHERA_IMPRISON_LINES = [
    `[집착] {{A:은/는}} {{B:을/를}} 자신의 방에 가두었다.`,
    `[경계] {{A:은/는}} {{B:을/를}} 문밖으로 한 발자국도 못 나가게 했다.`,
    `[독점] {{A:은/는}} {{B:이/가}} 다른 사람과 어울리는 것이 참을 수 없었다.`,
    `[불안] {{A:은/는}} {{B:이/가}} 자신을 떠날까 봐 방에 묶어두기로 했다.`,
    `[소유] {{A:은/는}} {{B:은/는}} 자신만 바라봐야 한다고 생각했다.`,
    `[광기] {{A:의}} 눈빛이 흔들렸다. 결국 {{B:은/는}} 방 안에 갇혔다.`,
    `[혼란] {{A:은/는}} 설명할 수 없는 충동에 휩싸여 {{B}}를 붙잡아 두었다.`,
    `[탈선] {{A:은/는}} {{B:에게}} '조금만 같이 있어 달라'며 문을 잠궜다.`,
    `[집착폭주] {{A:은/는}} {{B:을/를}} 누구에게도 빼앗기고 싶지 않았다.`,
    `[위험신호] {{A:은/는}} {{B:이/가}} 다른 사람과 웃는 것이 견딜 수 없었다.`,
    `[격리] {{A:은/는}} {{B:이/가}} 자신만 바라볼 때까지 시간을 벌기로 했다.`,
    `[정서불안] {{A:은/는}} {{B:을/를}} 방에 가둬 두지 않으면 불안함을 견딜 수 없었다.`,
];

//문구치환함수 
function formatImprisonLine(template, A, B) {
    return template.replace(/{{(A|B):([^}]+)}}/g, (_, who, josa) => {
        const name = who === "A" ? A : B;
        return name + getJosa(name, josa);
    });
}
